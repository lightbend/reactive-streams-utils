...
public class CollectSubscriberStageVerification extends AbstractStageVerification {

  CollectSubscriberStageVerification(ReactiveStreamsTck.VerificationDeps deps) {
    super(deps);
  }
  ...
  public void finisherFunctionShouldBeInvoked() {
    assertEquals(await(ReactiveStreams.of("1", "2", "3")
        .to(ReactiveStreams.collect(Collectors.joining(", "))).build(engine)), "1, 2, 3");
  }
  ...
  class SubscriberVerification extends StageSubscriberBlackboxVerification<Integer> {
    ...
    public Flow.Subscriber<Integer> createFlowSubscriber() {
      return ReactiveStreams.<Integer>toList().build(engine).getSubscriber();
    }
